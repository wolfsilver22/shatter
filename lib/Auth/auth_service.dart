...
class AuthService with ChangeNotifier {
  ...
  // ✅ دالة محسنة ومتوافقة تماماً مع قاعدة البيانات
  Future<List<Map<String, dynamic>>?> getCoursesForGrade(String gradeLevel) async {
    try {
      print('🎯 جلب الكورسات للصف: $gradeLevel');
      print('🔍 نوع بيانات gradeLevel: ${gradeLevel.runtimeType}');

      // ✅ التأكد من أن gradeLevel هو نص (String) ومتطابق مع قاعدة البيانات
      String searchLevel = gradeLevel.toString().trim();
      print('🔍 البحث بمستوى الصف: "$searchLevel"');

      final QuerySnapshot querySnapshot = await _firestore
          .collection('courses')
          .where('student_level', isEqualTo: searchLevel)
      ...
      if (querySnapshot.docs.isEmpty) {
        print('⚠️ لا توجد كورسات للصف: "$searchLevel"');

        // ✅ فحص تشخيصي إضافي
        await _debugAllCourses();
        await debugAvailableGradeLevels();
        return [];
      }
