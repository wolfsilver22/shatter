...
class AuthService with ChangeNotifier {
  ...
  // ✅ دالة محسّنة تعتمد على _selectedGrade مباشرة دون الحاجة إلى متغيرات إضافية
  Future<List<Map<String, dynamic>>?> getCoursesForCurrentGrade() async {
    try {
      ...
      final String searchLevel = _selectedGrade!.trim();
      print('🎯 جلب الكورسات للصف المحدد: "$searchLevel"');

      final QuerySnapshot querySnapshot = await _firestore
          .collection('courses')
          .where('student_level', isEqualTo: int.tryParse(searchLevel) ?? searchLevel)
      ...
      if (querySnapshot.docs.isEmpty) {
        print('⚠️ لا توجد كورسات متاحة للصف: "$searchLevel"');
