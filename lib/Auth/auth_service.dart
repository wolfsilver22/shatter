...
class AuthService with ChangeNotifier {
  ...
  Future<void> _initialize() async {
    try {
      ...
      if (currentUser != null) {
        _isLoggedIn = true;
        _studentEmail = currentUser.email;
        print('🔄 AuthService - جاري تحميل بيانات المستخدم...');
        await _loadUserData();
        print('✅ AuthService - اكتمل تحميل بيانات المستخدم');
      } else {
        _isLoggedIn = false;
        print('👤 AuthService - لا يوجد مستخدم مسجل');
      }
      notifyListeners();
    } catch (e) {
      _isLoggedIn = false;
      print('❌ AuthService - خطأ في التهيئة: $e');
      notifyListeners();
    }
  }
  ...
  Future<void> _loadUserData() async {
    try {
      if (_studentEmail != null) {
        print('📥 جاري تحميل بيانات المستخدم من Firestore: $_studentEmail');
        final doc = await _firestore
            .collection('user_settings')
            .doc(_studentEmail)
            .get();
        ...
        if (doc.exists) {
          ...
          print('✅ تم تحميل بيانات المستخدم - الصف: $_selectedGrade');
          notifyListeners();
        } else {
  ...
  Future<void> signInWithEmailAndPassword(String email, String password) async {
    try {
      ...
      print('✅ تسجيل الدخول ناجح - الصف المحدد: $_selectedGrade');
      notifyListeners();
  ...
  Future<List<Map<String, dynamic>>?> getCoursesForCurrentGrade() async {
    try {
      if (_selectedGrade == null) {
        print('❌ لا يمكن جلب المواد - الصف غير محدد');
        return [];
      }

      final int searchLevel = _selectedGrade!;
      print('📚 جاري جلب المواد للصف: $searchLevel');
      ...
      try {
        query = query.where('student_level', isEqualTo: searchLevel);
        print('✅ تم تطبيق فلترة المستوى: $searchLevel');
      } catch (e) {
        print('⚠️ لا يمكن تطبيق فلترة المستوى: $e');
        // تجاهل الخطأ والمتابعة بدون فلترة المستوى
      }

      try {
        query = query.orderBy('order_index');
        print('✅ تم تطبيق الترتيب حسب order_index');
      } catch (e) {
        print('⚠️ لا يمكن تطبيق الترتيب: $e');
        // تجاهل الخطأ والمتابعة بدون ترتيب
      }

      final QuerySnapshot querySnapshot = await query.get();
      print('✅ تم جلب ${querySnapshot.docs.length} مادة من Firestore');

      if (querySnapshot.docs.isEmpty) {
        print('⚠️ لم يتم العثور على مواد، جاري استخدام الطريقة البديلة...');
        return await _getCoursesFallback(searchLevel);
      }
      ...
      // ترتيب المواد حسب order_index
      courses.sort((a, b) => (a['order_index'] ?? 0).compareTo(b['order_index'] ?? 0));

      print('✅ تم تحضير ${courses.length} مادة للعرض');
      return courses;

    } catch (e) {
      print('❌ خطأ في جلب المواد: $e');
      if (_selectedGrade != null) {
        return await _getCoursesFallback(_selectedGrade!);
      }
      return [];
  ...
  Future<List<Map<String, dynamic>>?> _getCoursesFallback(int gradeLevel) async {
    try {
      print('🔄 استخدام الطريقة البديلة لجلب المواد للمستوى: $gradeLevel');
      ...
      // ترتيب المواد حسب order_index
      filteredCourses.sort((a, b) => (a['order_index'] ?? 0).compareTo(b['order_index'] ?? 0));

      print('✅ الطريقة البديلة وجدت ${filteredCourses.length} مادة');
      return filteredCourses;
  ...
  Future<void> setSelectedGrade(String grade) async {
    ...
    _selectedGrade = gradeInt;
    print('🎯 تم تحديث الصف الدراسي إلى: $gradeInt');
    ...
    try {
      ...
      print('⚠️ خطأ في تحديث الصف في Firestore: $e');
      // تجاهل الخطأ في التحديث
    }
