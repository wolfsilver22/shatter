...
//   }
//
//   // دالة لعرض نافذة تفعيل الاشتراك
...
//                       child: ElevatedButton(
//                         onPressed: () async {
//                           // هنا سيتم إضافة منطق التفعيل لاحقاً
//                           Navigator.pop(context);
...
//                 onPressed: () {
//                   Navigator.pop(context);
//                   _checkSubscriptionStatus(); // تحديث حالة الاشتراك
//                 },
//                 style: ElevatedButton.styleFrom(
//                   backgroundColor: _activeColor,
//                   foregroundColor: Colors.white,
...
class _MainNavigationState extends State<MainNavigation> {
  ...
  Future<void> _initializeUserData() async {
    try {
      ...
      if (authService.isLoggedIn) {
        ...
        // إذا لم يكن هناك اشتراك نشط، عرض الدايلوج الإلزامي
        if (!_subscriptionStatus['isActive'] && mounted) {
          print('🔒 لا يوجد اشتراك نشط - عرض الدايلوج الإلزامي');
          await Future.delayed(Duration(milliseconds: 500)); // تأخير بسيط للتهيئة
          if (mounted) {
            setState(() {
  ...
  void showMandatorySubscriptionDialog() {
    showDialog(
      context: context,
      barrierDismissible: false, // ⚠️ إلزامي - لا يمكن إغلاقه
      builder: (context) => MandatorySubscriptionDialog(
        onSubscriptionActivated: () {
          // عند تفعيل الاشتراك بنجاح
          setState(() {
            _showMandatorySubscriptionDialog = false;
          });
          _checkSubscriptionStatus(); // تحديث الحالة
        },
      ),
    );
  }
  ...
  Widget _buildCurrentScreen() {
    // إذا كان هناك دايلوج إلزامي، منع الوصول للشاشات
    if (_showMandatorySubscriptionDialog) {
      return Container(); // شاشة فارغة أثناء عرض الدايلوج
    }
  ...
  Widget build(BuildContext context) {
    return Consumer<AuthService>(
      builder: (context, authService, child) {
        // عرض الدايلوج الإلزامي إذا لزم الأمر
        ...
        return Directionality(
          ...
          child: Scaffold(
            ...
            // ✅ إخفاء BottomNavigationBar إذا كان هناك دايلوج إلزامي
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            'يجب تفعيل الاشتراك لاستخدام التطبيق',
                            style: TextStyle(
                              fontSize: 14.sp,
                              color: Colors.white.withOpacity(0.9),
                              fontFamily: 'Tajawal',
              ...
              Padding(
                ...
                child: Column(
                  children: [
                    ...
                    SizedBox(height: 8.h),
                    Text(
                      'لا يمكنك استخدام التطبيق بدون اشتراك نشط. يرجى إدخال كود التفعيل لتفعيل الاشتراك والمتابعة.',
                      textAlign: TextAlign.center,
                      style: TextStyle(
                        fontSize: 14.sp,
                    ...
                    Container(
                      ...
                      child: Row(
                        children: [
                          ...
                          Expanded(
                            child: Text(
                              'يمكنك الحصول على كود التفعيل من المسؤول أو من خلال خدمة العملاء',
                              style: TextStyle(
                                fontSize: 12.sp,
  ...
  Future<void> _activateSubscription() async {
    ...
    try {
      ...
      if (result['success']) {
        ...
        // إغلاق الدايلوج واستدعاء callback النجاح
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('حدث خطأ أثناء التفعيل: $e'),
          backgroundColor: const Color(0xFFE53E3E),
        ),
      );
    }
