...
// subscription_service.dart - الإصدار المحسن
import 'package:cloud_firestore/cloud_firestore.dart';
import 'package:firebase_auth/firebase_auth.dart';
import 'dart:async';
...
class SubscriptionService {
  ...
  Map<String, dynamic>? _cachedSubscriptionData;
  DateTime? _lastCacheTime;
  static const Duration _cacheDuration = Duration(minutes: 5);

  // ✅ Stream لمتابعة التغييرات
  ...
  Future<Map<String, dynamic>> validateSubscriptionCode(String code) async {
    try {
      ...
      try {
        QuerySnapshot cachedQuery = await _firestore
            .collection('subscription_codes')
            .where('code', isEqualTo: code.trim())
            .where('is_used', isEqualTo: false)
            .limit(1)
        ...
        if (cachedQuery.docs.isNotEmpty) {
          ...
          DocumentSnapshot codeDoc = cachedQuery.docs.first;
          Map<String, dynamic> codeData = codeDoc.data() as Map<String, dynamic>;
          ...
          return {
            ...
            'codeData': codeData,
            'codeId': codeDoc.id,
            'message': 'كود الاشتراك صالح (من الذاكرة المؤقتة)',
            'isCached': true
          };
        }
      } catch (cacheError) {
        print('⚠️ خطأ في التخزين المؤقت: $cacheError');
      }
  ...
  // ✅ دالة محسنة للتحقق من حالة الاشتراك الحالية (بدون تحديث تلقائي)
  Future<Map<String, dynamic>> checkUserSubscription({bool forceRefresh = false}) async {
    try {
      User? user = _auth.currentUser;
      if (user == null) {
        print('❌ [SubscriptionService] لا يوجد مستخدم مسجل دخول');
      ...
      if (!forceRefresh &&
          _cachedSubscriptionData != null &&
          _lastCacheTime != null &&
          DateTime.now().difference(_lastCacheTime!) < _cacheDuration) {
        print('📦 [SubscriptionService] استخدام البيانات من الذاكرة المؤقتة');
      ...
      final result = {
        ...
        'message': isActive
            ? '✅ اشتراك نشط (متبقي $daysRemaining يوم)'
            : '❌ اشتراك منتهي',
        'requiresMandatoryActivation': isExpired,
      ...
      // ✅ محاولة استخدام التخزين المؤقت عند وجود خطأ
      if (_cachedSubscriptionData != null) {
        print('📦 [SubscriptionService] استخدام البيانات المخزنة مؤقتاً بسبب الخطأ');
        return {
          ..._cachedSubscriptionData!,
          'isCached': true,
          'hasError': true,
          'errorMessage': 'خطأ في الاتصال. عرض البيانات المخزنة محلياً'
        };
      }
  ...
  // ✅ دالة منفصلة لتحديث حالة الاشتراك المنتهي
  Future<void> updateExpiredSubscription(String subscriptionId) async {
    try {
      print('🔄 [SubscriptionService] تحديث حالة الاشتراك: $subscriptionId');
      ...
      print('✅ تم تحديث حالة الاشتراك');

      // مسح الذاكرة المؤقتة بعد التحديث
      _clearCache();

    } catch (e) {
      print('⚠️ خطأ في تحديث حالة الاشتراك: $e');
    }
  }

  // ✅ Stream لمتابعة التغييرات في اشتراك المستخدم
  Stream<Map<String, dynamic>>? getSubscriptionStream() {
    User? user = _auth.currentUser;
  ...
  // ✅ دالة للتحديث الفوري من السيرفر
  Future<Map<String, dynamic>> refreshSubscriptionFromServer() async {
    print('🔄 [SubscriptionService] تحديث البيانات من السيرفر');
    return await checkUserSubscription(forceRefresh: true);
  }

  // ✅ دالة الذاكرة المؤقتة المحسنة
  void _cacheSubscriptionData(Map<String, dynamic> data) {
    _cachedSubscriptionData = {
      ...data,
      'cachedAt': DateTime.now()
    };
    _lastCacheTime = DateTime.now();
    print('💾 [SubscriptionService] تم تخزين البيانات في الذاكرة المؤقتة');
  }
  ...
  void _clearCache() {
    _cachedSubscriptionData = null;
    _lastCacheTime = null;
    print('🗑️ [SubscriptionService] تم مسح الذاكرة المؤقتة');
  }

  // ✅ دالة للتحقق الإجباري من الاشتراك
  Future<bool> checkMandatorySubscription() async {
    try {
      Map<String, dynamic> subscriptionStatus = await checkUserSubscription();
...
// // subscription_service.dart - الإصدار المبسط
// import 'package:cloud_firestore/cloud_firestore.dart';
// import 'package:firebase_auth/firebase_auth.dart';
//
// class SubscriptionService {
...
//         return {
//           'success': false,
//           'message': 'كود الاشتراك غير صالح أو تم استخدامه'
//         };
//       }
//
//       DocumentSnapshot codeDoc = querySnapshot.docs.first;
...
//       int durationDays = codeData['duration_day'];
//       DateTime endDate = startDate.add(Duration(days: durationDays));
//       String planType = codeData['plan_type'] ?? 'مميز';
//
...
//
//       // تحديث حالة الكود
//       await _firestore.collection('subscription_codes')
//           .doc(codeId)
//           .update({
//         'is_used': true,
//         'used_at': Timestamp.now(),
...
//   }
//
//   // ✅ دالة التحقق من حالة الاشتراك (المطلوبة فقط)
//   Future<Map<String, dynamic>> checkUserSubscription() async {
//     try {
//       User? user = _auth.currentUser;
//       if (user == null) {
...
//       Map<String, dynamic> currentSubscription = await checkUserSubscription();
//       DateTime newStartDate = currentSubscription['isActive']
//           ? (currentSubscription['endDate'] as DateTime)
//           : DateTime.now();
//
//       String codeId = validationResult['codeId'];
//       Map<String, dynamic> codeData = validationResult['codeData'];
