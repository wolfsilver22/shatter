...
class _LessonsListScreenState extends State<LessonsListScreen> {
  ...
  // ✅ التعديل: جلب الدروس من Firebase
  Future<void> _loadLessonsFromFirebase() async {
    ...
    try {
      ...
      // ✅ استخدام دالة AuthService لجلب الدروس
      final lessonsData = await authService.getLessonsForSubject(widget.tableName);
...
class __AnimatedLessonCardState extends State<_AnimatedLessonCard>
  ...
  @override
  Widget build(BuildContext context) {
    // ✅ التعديل: استخدام الحقول المناسبة من Firebase
    String lessonImage = widget.lesson['lesson_image'] ?? widget.lesson['image_url'] ?? '';
    String lessonTitle = widget.lesson['lesson_title'] ?? widget.lesson['title'] ?? 'بدون عنوان';
