...
class _HomeScreenState extends State<HomeScreen> with TickerProviderStateMixin {
  ...
  // ✅ التعديل: الحصول على مستوى الطالب ثم تحميل البيانات من Firebase
  ...
  Future<void> _checkConnectivityAndLoadData() async {
    try {
      ...
      // يوجد اتصال، جلب البيانات من Firebase
  ...
  // ✅ التعديل: جلب الكورسات من Firebase مع التصفية حسب student_level
  Future<void> _loadCoursesFromFirebase() async {
    ...
    try {
      ...
      // ✅ استخدام دالة AuthService لجلب الكورسات
      final courses = await authService.getCoursesForGrade(_studentLevel!);
  ...
  // ✅ دايلوج خطأ الاتصال بالإنترنت (يبقى كما هو)
  void _showNetworkErrorDialog() {
    showDialog(
      context: context,
      barrierColor: Colors.black54,
  ...
  // باقي دوال الـ UI تبقى كما هي بدون تغيير
