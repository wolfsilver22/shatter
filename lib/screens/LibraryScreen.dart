...
class _LibraryScreenState extends State<LibraryScreen> with TickerProviderStateMixin {
  ...
  Future<void> _loadData() async {
    try {
      ...
      // تحميل العروض
      final QuerySnapshot offersSnapshot = await _offersCollection
          .orderBy('created_at', descending: true)
          .get();

      // تحميل الكتب
      final QuerySnapshot booksSnapshot = await _booksCollection
          .orderBy('created_at', descending: true)
          .get();
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('حدث خطأ في تحميل البيانات: $error'),
          backgroundColor: Colors.red,
        ),
      );
    } finally {
  ...
  Widget _buildLibraryContent() {
    ...
    if (_books.isEmpty) {
      return SliverToBoxAdapter(
        child: _buildEmptyState(
          icon: Icons.library_books,
          title: 'لا توجد كتب حالياً',
          description: 'سيتم إضافة الكتب التعليمية قريباً',
          buttonText: 'تحديث',
          onTap: _refreshData,
        ),
      );
              ...
              children: [
                ...
                Expanded(
                  child: ClipRRect(
                    ...
                    child: Container(
                      color: Colors.white,
                      child: book['image_url'] != null && book['image_url'].isNotEmpty
                          ? Image.network(
                        book['image_url'],
                        fit: BoxFit.contain,
                        width: double.infinity,
            ...
            children: [
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  color: Colors.white,
                  image: book['image_url'] != null && book['image_url'].isNotEmpty
                      ? DecorationImage(
                ...
                child: book['image_url'] == null || book['image_url'].isEmpty
                    ? Center(
                  child: Icon(
                    Icons.menu_book_rounded,
                    size: 80.w,
                  ...
                  child: SingleChildScrollView(
                    child: Column(
                      ...
                      children: [
                        ...
                        Row(
                          children: [
                            _buildBookInfoItem(Icons.category, book['category'] ?? 'عام'),
                            SizedBox(width: 16.w),
                            _buildBookInfoItem(Icons.book, '${book['pages'] ?? 0} صفحة'),
                          ],
                        ),
                      ],
                    ),
  ...
  Widget _buildBookInfoItem(IconData icon, String text) {
    return Row(
      children: [
