...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  // دالة بديلة بإمكانيات إضافية (اختيارية)
  String formatDateWithCustomFormat(DateTime date, {String separator = '/'}) {
    int year = date.year;
    int month = date.month;
    int day = date.day;
  ...
  // دالة لعرض التاريخ باللغة العربية (اختيارية)
  ...
  Widget _buildActiveSubscription() {
    ...
    return Column(
      children: [
            ...
            children: [
              LayoutBuilder(
                builder: (context, constraints) {
                  double progress = daysRemaining / 30;
                  if (progress > 1) progress = 1.0;
        ...
        _buildSubscriptionInfoRow('الحالة', 'نشط'),
        _buildSubscriptionInfoRow('الأيام المتبقية', '$daysRemaining يوم'),
        _buildSubscriptionInfoRow(
            'تاريخ الانتهاء',
            formatDate(endDate) // استخدام الدالة البديلة
        ),
        ...
        Container(
          ...
          child: Row(
            children: [
              Icon(Iconsax.tick_circle, color: const Color(0xFF10B981), size: 18.sp),
              SizedBox(width: 8.w),
              Expanded(
                child: Text(
                  'اشتراكك نشط ومتجدد تلقائياً',
                  style: TextStyle(
                    fontSize: 14.sp,
  ...
  // باقي الأقسام الحالية (بدون تغيير)
  Widget _buildProfileHeader() {
    final authService = Provider.of<AuthService>(context, listen: false);
