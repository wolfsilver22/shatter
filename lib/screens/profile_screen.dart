...
class _ProfileScreenState extends State<ProfileScreen> with TickerProviderStateMixin {
  ...
  // ✅ التعديل: دالة لجلب بيانات الاشتراك مباشرة من جدول users
  Future<void> _loadSubscriptionData() async {
    try {
      final authService = Provider.of<AuthService>(context, listen: false);

      // التحقق من أن المستخدم مسجل الدخول
      if (!authService.isLoggedIn || authService.studentNumber == null) {
        setState(() {
          _isLoading = false;
          _subscriptionDays = 0;
        });
      ...
      // ✅ جلب بيانات subscription_days مباشرة من جدول users
      final response = await authService.supabase
          .from('users')
          .select('subscription_days')
          .eq('student_number', authService.studentNumber!)
          .single();

      setState(() {
        _subscriptionDays = response['subscription_days'] ?? 0;
        _isLoading = false;
      });
