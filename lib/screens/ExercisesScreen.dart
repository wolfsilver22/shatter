...
class _HomeworkSolverScreenState extends State<HomeworkSolverScreen>
  ...
  Future<void> _loadApiKey() async {
    ...
    try {
      _apiKey = await GeminiService.getApiKey(_firestore);

      if (_apiKey == null || _apiKey!.isEmpty) {
        throw Exception('لم يتم العثور على مفتاح API في قاعدة البيانات');
      }
      ...
      setState(() {
        _isLoadingApiKey = false;
        _errorMessage = 'خطأ في تحميل مفتاح API: $e';
      });
    }
  }
  ...
  Future<void> _solveHomework() async {
    ...
    if (_apiKey == null || _apiKey!.isEmpty) {
      setState(() {
        _errorMessage = 'مفتاح API غير متوفر. يرجى المحاولة مرة أخرى';
      });
      await _loadApiKey();
      return;
    }
    ...
    try {
      final solutionText = await GeminiService.solveHomework(_selectedImage!, _apiKey!);
  ...
  Widget _buildApiKeyStatus() {
    ...
    if (_apiKey == null && _errorMessage.isNotEmpty) {
      return Container(
        margin: EdgeInsets.symmetric(horizontal: 20.w, vertical: 8.h),
        padding: EdgeInsets.all(16.w),
        decoration: BoxDecoration(
  ...
  Widget _buildActionButton() {
    if (_isLoadingApiKey || (_apiKey == null && _errorMessage.isNotEmpty)) {
      return Container();
    }
  ...
  Widget _buildErrorMessage() {
    if (_errorMessage.isEmpty || _isLoadingApiKey) return Container();
...
class GeminiService {
  static Future<String> getApiKey(FirebaseFirestore firestore) async {
    try {
      ...
      if (snapshot.exists && snapshot.data() != null) {
        ...
        return data['api_key']?.toString() ?? '';
      } else {
        throw Exception('لم يتم العثور على إعدادات المفتاح في قاعدة البيانات');
      }
    } catch (e) {
      throw Exception('فشل في جلب المفتاح من قاعدة البيانات: $e');
    }
  }

  static Future<String> solveHomework(File image, String apiKey) async {
    try {
